...
Steam 愿望单价格监控插件。监控单个游戏或整个 Steam 公开愿望单的价格,当折扣达到设定阈值(或触及插件观测到的历史最低价)时,自动推送提醒到绑定的会话,消息附带游戏头图、原价、现价与史低标记。

## 功能特性

- **单游戏监控**: 通过商店链接或 AppID 添加任意游戏
- **愿望单一键导入**: 支持完整愿望单链接(数字 ID 或自定义 URL),后台自动建立价格基线
- **折扣阈值推送**: 折扣百分比达到阈值才提醒,避免小额折扣打扰
- **史低检测**: 插件持续记录观测期内最低价,触及史低时特别标记
...
git clone https://github.com/dsh/astrbot_plugin_steam_wishlist
```
...
/sw add https://store.steampowered.com/app/620/   # 添加单游戏
/sw import https://store.steampowered.com/wishlist/profiles/你的SteamID/   # 导入整个愿望单
```

绑定并添加监控后,插件会按配置的间隔自动检查价格并推送折扣提醒。
...
1. 打开自己的 Steam 愿望单页面,浏览器地址形如
   `https://store.steampowered.com/wishlist/profiles/76561198xxxxxxxxxx/`,直接复制即可;
2. 若你使用自定义 URL(`wishlist/id/yourname/` 形式),插件会自动解析;
3. 也可以直接粘贴 17 位 SteamID 数字。
...
所有数据(监控列表、价格状态、推送绑定)保存在 AstrBot 数据目录
